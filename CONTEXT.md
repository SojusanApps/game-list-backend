...
The owner's own 1-10 number for a game, set per `GameList` entry. Optional: an entry can exist with no Score (a `Plan to Play` game, say), and an unscored entry means "no opinion yet" — never "0" and never "the lowest score". Independent of **Recommendation**. Distinct from a game's **average score**, the community-wide mean of every user's Score for it, which drives the game's rank in the catalogue; a user's own list is ordered by that user's Score, not by the community's.
_Avoid_: rating, grade (for the per-entry number); rank (that is the community position derived from the average score)

**Not planned** (`GameList.status = "NP"`):
A `GameList` entry for a game the user has added to their list — most often via **Steam Import** or **Title Import** — but does not intend to ever play. Distinct from `Plan to play` (not yet played, but the user does intend to) and `Dropped` (started playing, then abandoned). The point of adding it at all, rather than leaving the game off the list, is to record that the user has or recognizes the game without claiming any intent to play it. A general status like every other `GameList.status` value: assignable and re-settable on any entry via the normal create/update flow, with `Score` remaining optional and, if set, still counted toward the game's `average score` — see [ADR-0020](docs/adr/0020-not-planned-games-count-toward-gamestats.md).
_Avoid_: ignored, skipped, backlog (backlog implies eventual intent to play — that's `Plan to play`)
...
**Review language** (`GameReview.language`):
A mandatory field on every `GameReview`: `English` or `Polish`, the language the review text is written in, chosen explicitly by the reviewer. It labels the text only — the system does not detect or verify it against what was written. A User still has at most one `GameReview` per `Game`, so the Review language is a changeable attribute of that one review, not a way to hold parallel reviews in both languages. Readers can filter reviews by it; with no filter, reviews in every language are returned. Distinct from the UI locale (the request's active language), which drives translations of `Game` and **Lookup Model** names and is never applied to filter reviews automatically.
_Avoid_: locale, audience, translation (a review is written once in one language, never translated)
...
**Favorite**:
A **User**'s own private bookmark on a `Collection`, belonging to that User rather than to the Collection — each User's Favorites are independent, so the Owner favoriting their Collection does not favorite it for its collaborators or viewers. Any User who can see a Collection (its Owner, a collaborator, a friend of the Owner on a FRIENDS Collection, or anyone on a PUBLIC one) can favorite it, without needing write access; each viewer sees only their own state as `is_favorite`. Nobody else can see who favorited a Collection and there is no favorite count; an **Anonymous visitor** has no Favorites. Losing sight of a Collection (removed as collaborator, visibility tightened, friendship ended) keeps the Favorite dormant until access returns; it is discarded only when the Collection or the User is erased.
_Avoid_: star, like (implies a public tally), pin, bookmark
