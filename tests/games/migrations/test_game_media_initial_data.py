"""Tests for the GameMedia seeding RunPython functions in the games app's squashed initial migration.
...
INITIAL_DATA = _migration.GAME_MEDIA_INITIAL_DATA


@pytest.mark.django_db()
def test_populate_game_media_creates_the_initial_records() -> None:
    """populate_game_media bulk-creates one GameMedia row per entry in INITIAL_DATA, with name_en set.

    The migration already seeded these rows when the test database was set up, so the table
    is cleared first to exercise a clean insert, matching what happens the first time this
    migration ever runs.
    """
    ...
    created_names_en = set(GameMedia.objects.values_list("name_en", flat=True))
    expected_names = {data["name"] for data in INITIAL_DATA}
    assert expected_names.issubset(created_names)
    assert expected_names.issubset(created_names_en)
