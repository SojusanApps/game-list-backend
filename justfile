...
fresh_run:
    uv run scripts/game-list-manage.py collectstatic
    uv run scripts/game-list-manage.py makemigrations
...
translations:
    uv run scripts/game-list-manage.py makemessages -l pl

# Compile translation messages
finish_translations:
    uv run scripts/game-list-manage.py compilemessages
...
generate_openapi:
    uv run scripts/game-list-manage.py spectacular --format openapi-json --file openapi.json

# Run a Docker container with the app database
app_db:
    docker run --name dev-game-list-postgres \
        -p 5432:5432 \
        -e POSTGRES_DB=game_list \
        -e POSTGRES_USER=game_list \
        -e POSTGRES_PASSWORD=game_list \
        -v dev-game-list-postgres-data:/var/lib/postgresql \
...
tui:
    uv run scripts/game-list-manage.py custom_tui

# Import data from IGDB to the application database
import_igdb_data:
    uv run scripts/game-list-manage.py import_data_from_igdb platforms genres game_modes player_perspectives \
...
recalculate_stats:
    uv run scripts/game-list-manage.py recalculate_stats
